...
def test_step_02():
    ...
    if hasattr(mask_module, 'causal_mask'):
        ...
        if '@F.functional' in func_source:
            ...
            print("   Hint: Add '@F.functional' decorator above the function definition")
    else:
    ...
    if 'F.band_part' in source:
        if 'num_lower=None' in source and 'num_upper=0' in source and 'exclude=True' in source:
            print("✅ F.band_part is used with correct parameters (num_lower=None, num_upper=0, exclude=True)")
        else:
            print("❌ F.band_part is not used with correct parameters")
            print("   Hint: Use F.band_part(mask, num_lower=None, num_upper=0, exclude=True)")
    else:
    ...
    lines = source.split('\n')
    none_assignments = [line for line in lines if 'mask = None' in line or 'return None' in line]
    ...
    try:
        ...
        if hasattr(result, 'shape'):
            ...
            if actual_shape == expected_shape:
                ...
                print(f"❌ Mask shape is incorrect: expected {expected_shape}, got {actual_shape}")

    except ImportError:
        print("⚠️  Cannot test function execution (MAX not available in this environment)")
